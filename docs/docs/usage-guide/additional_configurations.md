...
The possible configurations of Pr Merge are stored in [here](https://github.com/Khulnasoft/pr-insight/blob/main/pr_insight/settings/configuration.toml){:target="_blank"}.
...
![possible_config1](https://khulnasoft/images/pr_insight/possible_config1.png){width=512}
...
![possible_config2](https://khulnasoft/images/pr_insight/possible_config2.png){width=512}


## Ignoring files from analysis

In some cases, you may want to exclude specific files or directories from the analysis performed by Pr Merge. This can be useful, for example, when you have files that are generated automatically or files that shouldn't be reviewed, like vendor code.
...
All Pr Merge tools have a parameter called `extra_instructions`, that enables to add free-text extra instructions. Example usage:
```
...
The default response language for Pr Merge is **U.S. English**. However, some development teams may prefer to display information in a different language. For example, your team's workflow might improve if PR descriptions and code suggestions are set to your country's native language.
...
Pr Merge will try to increase the number of lines of context, via the parameter:
```
[config]
...
If the PR is too large (see [PR Compression strategy](https://github.com/Khulnasoft/pr-insight/blob/main/PR_COMPRESSION.md)), Pr Merge may automatically set this number to 0, and will use the original git patch.

## Log Level

Pr Merge allows you to control the verbosity of logging by using the `log_level` configuration parameter. This is particularly useful for troubleshooting and debugging issues with your PR workflows.
...
Pr Merge allows you to automatically ignore certain PRs based on various criteria:
...
Pr Merge automatically identifies and ignores pull requests created by bots using:
