...
from pr_insight.algo import (CLAUDE_EXTENDED_THINKING_MODELS,
                             NO_SUPPORT_TEMPERATURE_MODELS,
                             SUPPORT_REASONING_EFFORT_MODELS,
                             USER_MESSAGE_ONLY_MODELS)
from pr_insight.algo.ai_handlers.base_ai_handler import BaseAiHandler
from pr_insight.algo.utils import ReasoningEffort, get_version
        ...
        try:
            ...
            if img_path:
                try:
                    ...
                    if r.status_code == 404:
                        error_msg = f"The image link is not [alive](img_path).\nPlease repost the original image as a comment, and send the question again with 'quote reply' (see [instructions](https://pr-insight-docs.khulnasoft/tools/ask/#ask-on-images-using-the-pr-code-as-context))."
            ...
            if model in self.support_reasoning_models:
                ...
                reasoning_effort = (
                    ...
                    if (get_settings().config.reasoning_effort in supported_reasoning_efforts)
