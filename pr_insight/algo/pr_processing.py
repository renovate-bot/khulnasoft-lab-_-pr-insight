...
def pr_generate_compressed_diff(
    ...
    total_tokens, patches, remaining_files_list, files_in_patch_list = generate_full_patch(
        convert_hunks_to_line_numbers, file_dict, max_tokens_model, remaining_files_list, token_handler
    )
...
def generate_full_patch(
    convert_hunks_to_line_numbers, file_dict, max_tokens_model, remaining_files_list_prev, token_handler
):
    total_tokens = token_handler.prompt_tokens  # initial tokens
    patches = []
        ...
        if (
            ...
            if get_settings().config.get("large_patch_policy", "skip") == "skip":
                ...
                patch_clipped = clip_tokens(
                    patch, delta_tokens, delete_last_line=True, num_input_tokens=new_patch_tokens
                )
...
def add_ai_metadata_to_diff_files(git_provider, pr_description_files):
    ...
    try:
        ...
        get_logger().error(
            f"Failed to add AI metadata to diff files: {e}", artifact={"traceback": traceback.format_exc()}
        )
...
def add_ai_summary_top_patch(file, full_extended_patch):
    try:
        ...
        get_logger().error(
            f"Failed to add AI summary to the top of the patch: {e}", artifact={"traceback": traceback.format_exc()}
        )
        return full_extended_patch
