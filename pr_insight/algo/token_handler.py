            ...
            with cls._lock:  # Lock acquisition to ensure thread safety
                if cls._encoder_instance is None or model != cls._model:
                    ...
                    cls._encoder_instance = (
                        encoding_for_model(cls._model) if "gpt" in cls._model else get_encoding("cl100k_base")
                    )
...
class TokenHandler:
    ...
    def estimate_token_count_for_non_anth_claude_models(self, model, default_encoder_estimate):
        ...
        if ("gpt" in get_settings().config.model.lower() or model_is_from_o_series) and get_settings(
            use_context=False
        ).get("openai.key"):
        ...
        get_logger().warning(
            f"{model}'s expected token count cannot be accurately estimated. Using {elbow_factor} of encoder output as best effort estimate"
        )
