...
from pr_insight.log import setup_logger
...
def run(inargs=None, args=None):
    ...
    if args.issue_url:
        result = asyncio.run(PRInsight().handle_request(args.issue_url, [command] + args.rest))
    else:
        result = asyncio.run(PRInsight().handle_request(args.pr_url, [command] + args.rest))
    if not result:
        parser.print_help()
