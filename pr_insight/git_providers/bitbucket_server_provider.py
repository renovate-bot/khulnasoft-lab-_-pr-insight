...
class BitbucketServerProvider(GitProvider):
    def __init__(
        ...
        self.bitbucket_client = bitbucket_client or Bitbucket(
            url=self.bitbucket_server_url, token=get_settings().get("BITBUCKET_SERVER.BEARER_TOKEN", None)
        )
        try:
    ...
    ) -> str:  # bitbucket server does not support issue url, so ignore param
        try:
    ...
    def get_canonical_url_parts(self, repo_git_url: str = None, desired_branch: str = None) -> Tuple[str, str]:
        ...
        if not repo_git_url:
            ...
            if "displayId" in default_branch_dict:
                ...
                get_logger().error(
                    f"Cannot obtain default branch for workspace_name={workspace_name}, "
        ...
        if not workspace_name or not project_name:
            get_logger().error(
                f"workspace_name or project_name not found in context, either git url: {repo_git_url} or uninitialized workspace/project."
            )
            return ("", "")
    ...
    def get_repo_settings(self):
        try:
            content = self.bitbucket_client.get_content_of_file(
                self.workspace_slug, self.repo_slug, ".pr_insight.toml", self.get_pr_branch()
            )
    ...
    def get_diff_files(self) -> list[FilePatchInfo]:
        ...
        if self.bitbucket_api_version is not None and self.bitbucket_api_version >= parse_version("8.16"):
            ...
            source_commits_list = list(
                self.bitbucket_client.get_pull_requests_commits(self.workspace_slug, self.repo_slug, self.pr_num)
            )
                ...
                try:
                    destination_commits = list(
                        self.bitbucket_client.get_commits(
                            self.workspace_slug, self.repo_slug, base_sha, self.pr.toRef["latestCommit"]
                        )
                    )
    ...
    def _clone_inner(self, repo_url: str, dest_folder: str, operation_timeout_in_seconds: int = None):
        ...
        cli_args = shlex.split(
            f"git clone -c http.extraHeader='Authorization: Bearer {bearer_token}' "
