...
async def mark_notification_as_read(headers, notification, session):
    async with session.patch(
        f"https://api.github.com/notifications/threads/{notification['id']}", headers=headers
...
async def async_handle_request(pr_url, rest_of_comment, comment_id, git_provider):
    ...
    success = await agent.handle_request(
        pr_url, rest_of_comment, notify=lambda: git_provider.add_eyes_reaction(comment_id)
    )
    return success
...
async def is_valid_notification(notification, headers, handled_ids, session, user_id):
    try:
                ...
                async with session.get(latest_comment, headers=headers) as comment_response:
                    ...
                    if comment_response.status == 200:
                        ...
                        if not check_prev_comments:
                            ...
                            get_logger().warning(
                                f"Failed to fetch comments for PR: {pr_url}", artifact={"comments": comments}
                            )
                            return False, handled_ids
        ...
        get_logger().exception(
            f"Error processing polling notification", artifact={"notification": notification, "error": e}
        )
        return False, handled_ids
