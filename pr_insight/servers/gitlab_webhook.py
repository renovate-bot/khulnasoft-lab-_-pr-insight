...
async def _perform_commands_gitlab(commands_conf: str, agent: PRInsight, api_url: str, log_context: dict, data: dict):
    ...
    async def inner(data: dict):
        ...
        if request.headers.get("X-Gitlab-Token") and secret_provider:
            ...
            if not secret:
                ...
                return JSONResponse(
                    status_code=status.HTTP_401_UNAUTHORIZED, content=jsonable_encoder({"message": "unauthorized"})
                )
            try:
