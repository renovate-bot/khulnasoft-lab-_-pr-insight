...
class PRCodeSuggestions:
    def __init__(
        self, pr_url: str, cli_mode=False, args: list = None, ai_handler: partial[BaseAiHandler,] = LiteLLMAIHandler
    ):
        ...
        self.progress += f"""\nWork in progress ...<br>\n<img src="https://khulnasoft/images/pr_insight/dual_ball_loading-crop.gif" width=48>"""
    ...
    async def publish_no_suggestions(self):
        ...
        if get_settings().config.publish_output and get_settings().pr_code_suggestions.get(
        ...
        if not data["code_suggestions"]:
            ...
            if self.progress_response:
                return self.git_provider.edit_comment(
                    self.progress_response, body="No suggestions found to improve this PR."
                )
            else:
    ...
    async def _prepare_prediction_extended(self, model: str) -> dict:
        ...
        if get_settings().pr_code_suggestions.decouple_hunks:
            ...
            # non-decoupled hunks
            self.patches_diff_list_no_line_numbers = get_pr_multi_diffs(
                self.git_provider,
                self.token_handler,
                model,
                max_calls=get_settings().pr_code_suggestions.max_number_of_calls,
        ...
        with get_logger().contextualize(sub_feature="convert_to_decoupled_with_line_numbers"):
            try:
                ...
                get_logger().exception(
                    f"Error converting to decoupled with line numbers",
...
"""
    ...
    async def self_reflect_on_suggestions(
        ...
        try:
            ...
            if dedicated_prompt:
                system_prompt_reflect = environment.from_string(get_settings().get(dedicated_prompt).system).render(
                    variables
                )
                user_prompt_reflect = environment.from_string(get_settings().get(dedicated_prompt).user).render(
                    variables
                )
            else:
