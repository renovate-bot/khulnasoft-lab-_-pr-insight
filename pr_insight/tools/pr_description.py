            ...
            if self.git_provider.is_supported("gfm_markdown") and get_settings().pr_description.enable_help_text:
                ...
                if isinstance(self.git_provider, GithubProvider):
                    pr_body += (
                        ...
                        "in the comments thread for any questions about PR-Insight usage.</li><li>Check out the "
                        '<a href="https://pr-insight-docs.khulnasoft.com/usage-guide/">documentation</a> '
                        "for more information.</li></details>"
                    )
                else:  # gitlab
                    pr_body += (
                        "\n\n___\n\n<details><summary>Need help?</summary>- Type <code>/help how to ...</code> in the comments "
                        "thread for any questions about PR-Insight usage.<br>- Check out the "
                        "<a href='https://pr-insight-docs.khulnasoft.com/usage-guide/'>documentation</a> for more information.</details>"
                    )
    ...
    async def _prepare_prediction(self, model: str) -> None:
        ...
        if not large_pr_handling or patches_diff:
            ...
            if patches_diff:
                ...
                get_logger().error(
                    f"Error getting PR diff {self.pr_id}", artifact={"traceback": traceback.format_exc()}
                )
                self.prediction = None
        else:
    ...
    async def extend_uncovered_files(self, original_prediction: str) -> str:
        try:
            ...
            if original_prediction_dict:
                filenames_predicted = [
                    file.get("filename", "").strip() for file in original_prediction_dict.get("pr_files", [])
                ]
            else:
                filenames_predicted = []
