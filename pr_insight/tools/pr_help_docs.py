...
def extract_model_answer_and_relevant_sources(ai_response: str) -> str | None:
    ...
    # (for more details, see here: https://github.com/Khulnasoft/pr-insight-pro/blob/main/pr_insight/tools/pr_help_message.py#L173)
    """
    For example:
    ...
    if "### Answer:\n" in ai_response:
        ...
        if "#### Relevant Sources:\n\n" in model_answer_and_relevant_sources_sections_in_response:
            model_answer_section_in_response = model_answer_and_relevant_sources_sections_in_response.split(
                "#### Relevant Sources:\n\n"
            )[0]
            get_logger().info(f"Found model answer: {model_answer_section_in_response}")
            return (
                model_answer_and_relevant_sources_sections_in_response
                if len(model_answer_section_in_response) > 0
                else None
            )
    get_logger().warning(f"Either no answer section found, or that section is malformed: {ai_response}")
    return None
...
def map_documentation_files_to_contents(
    base_path: str, doc_files: list[str], max_allowed_file_len=5000
) -> dict[str, str]:
    try:
        returned_dict = {}
            ...
            try:
                with open(file, "r", encoding="utf-8") as f:
                    ...
                    if len(content) > max_allowed_file_len:
                        get_logger().warning(
                            f"File {file} length: {len(content)} exceeds limit: {max_allowed_file_len}, so it will be trimmed."
                        )
...
def aggregate_documentation_files_for_prompt_contents(
    file_path_to_contents: dict[str, str], return_just_headings=False
) -> str:
    try:
        docs_prompt = ""
...
def format_markdown_q_and_a_response(
    question_str: str,
    response_str: str,
    ...
    try:
        ...
        for section in relevant_sections:
            file = (
                section.get("file_name").lstrip("/").strip()
            )  # Remove any '/' in the beginning, since some models do it anyway
...
def clean_markdown_content(content: str) -> str:
    ...
    try:
        ...
        content = re.sub(
            r"<(?!table|tr|td|th|thead|tbody)([a-zA-Z][a-zA-Z0-9]*)[^>]*>(.*?)</\1>", r"\2", content, flags=re.DOTALL
        )
    ...
    async def __call__(self, model: str) -> str:
        ...
        try:
            ...
            get_logger().exception(
                "Caught exception during prediction.",
                artifacts={"system": self.system_prompt, "user": self.user_prompt},
            )
            raise e
...
class PRHelpDocs(object):
    def __init__(
        ...
        try:
            ...
            self.repo_desired_branch = get_settings().get(
            ...
            if not self.repo_url:
                ...
                get_logger().debug(
                    f"No explicit repo url provided, deducing it from type: {self.git_provider.__class__.__name__} "
            ...
            self.token_handler = TokenHandler(
                None, self.vars, get_settings().pr_help_docs_prompts.system, get_settings().pr_help_docs_prompts.user
            )
        except Exception as e:
            get_logger().exception(
                f"Caught exception during init. Setting self.question to None to prevent run() to do anything."
            )
            self.question = None
    ...
    async def run(self):
        ...
        try:
            ...
            invoke_llm_just_with_headings = self._trim_docs_input(
                docs_prompt_to_send_to_model, max_allowed_txt_input, only_return_if_trim_needed=True
            )
            ...
            if not response_str or not relevant_sections:
                get_logger().error(
                    "Failed to extract response/relevant sections.",
                    artifacts={
            ...
            if int(response_yaml.get("question_is_relevant", "1")) == 0:
                get_logger().warning(
                    f"Question is not relevant. Returning without an answer...", artifacts={"raw_response": response}
                )
                return
    ...
    def _find_all_document_files_matching_exts(
        self, abs_docs_path: str, ignore_readme=False, max_allowed_files=5000
    ) -> list[str]:
        try:
            matching_files = []
            ...
            for root, _, files in os.walk(abs_docs_path):
                for file in files:
                    if (
                        ignore_readme
                        and root == abs_docs_path
                        and file.lower() in [f"readme.{ext}" for ext in dotless_extensions]
                    ):
                        continue
        ...
        try:
            with TemporaryDirectory() as tmp_dir:
                ...
                if os.path.exists(abs_docs_path):
                    doc_files.extend(
                        self._find_all_document_files_matching_exts(
                            abs_docs_path, ignore_readme=(self.docs_path == ".")
                        )
                    )
                    if not doc_files:
                        get_logger().warning(
                            f"No documentation files found matching file extensions: "
                ...
                get_logger().info(
                    f"For context {self.ctx_url} and repo: {self.repo_url}"
    ...
    def _trim_docs_input(
        self, docs_input: str, max_allowed_txt_input: int, only_return_if_trim_needed=False
    ) -> bool | str:
        try:
            ...
            if token_count > max_tokens_full - delta_output:
                if only_return_if_trim_needed:
                    return True
                docs_input = clean_markdown_content(docs_input)  # Reduce unnecessary text/images/etc.
                get_logger().info(
                    f"Token count {token_count} exceeds the limit {max_tokens_full - delta_output}. Attempting to clip text to fit within the limit..."
                )
                docs_input = clip_tokens(docs_input, max_tokens_full - delta_output, num_input_tokens=token_count)
            if only_return_if_trim_needed:
                return False
            return docs_input
        except Exception as e:
    ...
    async def _rank_docs_and_return_them_as_prompt(
        self, docs_filepath_to_contents: dict[str, str], max_allowed_txt_input: int
    ) -> str:
        try:
            # Return just file name and their headings (if exist):
            docs_prompt_to_send_to_model = aggregate_documentation_files_for_prompt_contents(
                docs_filepath_to_contents, return_just_headings=True
            )
            # Verify list of headings does not exceed limits - trim it if it does.
            docs_prompt_to_send_to_model = self._trim_docs_input(
                docs_prompt_to_send_to_model, max_allowed_txt_input, only_return_if_trim_needed=False
            )
            if not docs_prompt_to_send_to_model:
            ...
            valid_indices = [
                int(entry["idx"])
                for entry in response_yaml.get("relevant_files_ranking")
    ...
    def _format_model_answer(self, response_str: str, relevant_sections: list[dict[str, str]]) -> str:
        try:
            canonical_url_prefix, canonical_url_suffix = self.git_provider.get_canonical_url_parts(
                repo_git_url=self.repo_url if self.repo_url_given_explicitly else None,
                desired_branch=self.repo_desired_branch,
            )
            answer_str = format_markdown_q_and_a_response(
                self.question,
                response_str,
                relevant_sections,
