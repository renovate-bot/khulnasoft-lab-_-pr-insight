...
class PRHelpMessage:
    def __init__(
        self, pr_url: str, args=None, ai_handler: partial[BaseAiHandler,] = LiteLLMAIHandler, return_as_string=False
    ):
        ...
        if self.question_str:
            ...
            self.token_handler = TokenHandler(
                None, self.vars, get_settings().pr_help_prompts.system, get_settings().pr_help_prompts.user
            )
        ...
        try:
            if self.question_str:
                ...
                md_files = [
                    ...
                    and not any(file.name == file_to_exclude for file_to_exclude in files_to_exclude)
                ]
                ...
                md_files_priority = [
                    file
                    for file in md_files
                    if any(priority_string in str(file) for priority_string in priority_files_strings)
                ]
                ...
                pr_comment += (
                    "Welcome to the PR Insight, an AI-powered tool for automated pull request analysis, feedback, suggestions and more."
                    ""
                )
                pr_comment += "\n\nHere is a list of tools you can use to interact with the PR Insight:\n"
                base_path = "https://pr-insight-docs.khulnasoft/tools"
                ...
                if isinstance(self.git_provider, GithubProvider) and not get_settings().config.get(
                    ...
                    pr_comment += "</table>\n\n"
                    pr_comment += f"""\n\n(1) Note that each tool can be [triggered automatically](https://pr-insight-docs.khulnasoft/usage-guide/automations_and_usage/#github-app-automatic-tools-when-a-new-pr-is-opened) when a new PR is opened, or called manually by [commenting on a PR](https://pr-insight-docs.khulnasoft/usage-guide/automations_and_usage/#online-usage)."""
                    ...
                    pr_comment += "</table>\n\n"
                    pr_comment += f"""\n\nNote that each tool can be [invoked automatically](https://pr-insight-docs.khulnasoft/usage-guide/automations_and_usage/) when a new PR is opened, or called manually by [commenting on a PR](https://pr-insight-docs.khulnasoft/usage-guide/automations_and_usage/#online-usage)."""
